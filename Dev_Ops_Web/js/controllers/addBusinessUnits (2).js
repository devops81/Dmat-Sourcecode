...
angular.module('genpactApp').controller('addBusinessUnitCtrl', function($scope,$http,commonDataFactory,appConfig) {
    ...
    $scope.resetValues=function(){
        $scope.businessUnitExist=false;
        $scope.businessUnitAlreadyExist=false;
    }
    ...
    $scope.addBusinessUnit = function() {
        ...
        for(var i=0;i<$scope.businessUnitList.length;i++){
            if($scope.businessUnitList[i].businessUnitName.toLowerCase() == addBusinessUnit.businessUnitName.toLowerCase()){
                $scope.businessUnitExist=true;
                break;
            }
        }
        if($scope.businessUnitExist!=true) {
    ...
    $scope.businessUnitAlreadyExistFunc=function(businessUnit){
        $scope.businessUnitAlreadyExist=false;
        for(var i=0;i<$scope.businessUnitList.length;i++){
            if($scope.businessUnitList[i].businessUnitName.toLowerCase()==businessUnit.toLowerCase()&&businessUnit.toLowerCase()!=$scope.oldBusinessUnit.toLowerCase()){
                $scope.businessUnitAlreadyExist=true;
            }
        }
    }
    $scope.updateBusinessUnitStatus = function(businessUnit) {
    	if($scope.businessUnitAlreadyExist!=true) {
    ...
    $scope.openBusinessUnitModel=function(businessUnit){
        ...
        $scope.oldBusinessUnit=businessUnit.businessUnitName;
    }
