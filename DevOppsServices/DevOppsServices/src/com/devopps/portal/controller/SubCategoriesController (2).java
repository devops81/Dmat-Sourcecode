...
public class SubCategoriesController extends AbstractController{
	...
	/*
	 * URL : http://localhost:8080/DevOppsServices/subCategories/getSubCategories/1.0/0
	 */
	@RequestMapping(value = "getSubCategories/{version}/{categoryId}", method = RequestMethod.POST, produces = "application/xml")
	public @ResponseBody String fetchSubCategories(HttpServletRequest request,@PathVariable int categoryId) {
		LOGGER.debug("[fetchSubCategories] Service :: Start");
		String indentedJson = null;
		SubCategoryServiceResponse response = null;
		response =subCategoryService.getSubCategories(categoryId);
