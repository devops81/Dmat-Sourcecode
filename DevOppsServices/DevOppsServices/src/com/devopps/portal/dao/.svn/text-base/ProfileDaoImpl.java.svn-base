...
public class ProfileDaoImpl implements IProfileDao{
	...
	public void saveUserProfile(UserProfileRequestBody userProfileRequestBody) {
		...
		if (userProfileRequestBody.getSaveOrUpdate() == 1) {
			...
			UserProfileEntity profileEntity = (UserProfileEntity) session.load(UserProfileEntity.class, userProfileRequestBody.getEmployeeId());
			List existingProfile = session.createCriteria(UserProfileEntity.class).add(Restrictions.eq("id",userProfileRequestBody.getEmployeeId())).list();
