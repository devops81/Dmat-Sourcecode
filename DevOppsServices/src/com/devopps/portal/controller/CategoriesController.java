...
public class CategoriesController extends AbstractController{
	...
	/**
	 * URL : http://localhost:8080/DevOppsServices/categories/getCategories/1.0/0/1
	 * 
	 * Response
	 * {"response": {
    "detail": {
	 ...
	 */
	@RequestMapping(value = "/getCategories/{version}/{assessmentId}/{questionnaireType}", method = RequestMethod.POST,  produces = "application/xml")
	public @ResponseBody String getCategories(HttpServletRequest request,@PathVariable int assessmentId,@PathVariable int questionnaireType) {
		LOGGER.debug("[getCategories] Service :: Start");
		CategoryServiceResponse response = null;
		response =categoryService.getCategories(assessmentId, questionnaireType);
		LOGGER.debug("[getCategories()] :: Ends :: Response -> "+response);
		String stringXML = marshal(response);
		LOGGER.debug("The string response " + stringXML);
