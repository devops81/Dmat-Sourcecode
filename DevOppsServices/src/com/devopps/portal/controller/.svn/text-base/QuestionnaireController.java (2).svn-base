...
public class QuestionnaireController extends AbstractController {
	...
	/**
	 * http://localhost:8080/DevOppsServices/questionnaire/viewQuestionnaire/1/1
	 * 
	 * 
	 */
	@RequestMapping(value="viewQuestionnaire/{version}/{questionnaireType}",method = RequestMethod.POST,produces = "application/xml")
	public @ResponseBody String viewQuestionnaire(HttpServletRequest request,@PathVariable Integer version,@PathVariable Integer questionnaireType) {
		LOGGER.debug("[viewQuestionnaire()] Service :: Start");
		ViewQuestionnaireResponse viewQuestionnaireResponse = null;
		String jsonResponse = null;
		viewQuestionnaireResponse = questionService.getQuestionDetails(questionnaireType);
		LOGGER.debug("[viewQuestionnaire()] :: Ends :: Response -> "+viewQuestionnaireResponse);
		String stringXML = marshal(viewQuestionnaireResponse);
		LOGGER.debug("The xml is" + stringXML);
