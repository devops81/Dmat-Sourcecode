...
public class SubCategoryDaoImpl implements ISubCategoryDao {
	...
	@Override
	public List<CategorySubCategoryDetails> getSubCategories(int categoryId, int questionnaireType) {
		Session session = getSessionFactory().openSession();
		List<Object[]> subCategoryInfo = null;
		int questionScoreCount = -1;
		if (categoryId == 0) {
			subCategoryInfo = (List<Object[]>)session.createQuery(IHibernateQueries.FETCH_SUB_CATEGORIES).setParameter("questionnaireType", questionnaireType).list();
		} else {
			subCategoryInfo = (List<Object[]>)session.createQuery(IHibernateQueries.FETCH_SUB_CATEGORIES_FILTERED).setParameter("categoryId", categoryId).setParameter("questionnaireType", questionnaireType).list();
		}
		final List<CategorySubCategoryDetails> subCategoryDetails = new ArrayList <CategorySubCategoryDetails>();
		CategorySubCategoryDetails categorySubCategoryInfo = null;
		int activeFlag = -1;
		for (Object[] subCategory : subCategoryInfo) {
			categorySubCategoryInfo = new CategorySubCategoryDetails();
			...
			questionScoreCount = session.createCriteria(QuestionEntity.class).add(Restrictions.eq("categoryId",(Integer)subCategory[0]))
					.add(Restrictions.eq("subcategoryId",(Integer)subCategory[2])).list().size();
			categorySubCategoryInfo.setEditAllowed(questionScoreCount > 0 ? false : true);
			categorySubCategoryInfo.setActive(activeFlag == 1 ? true : false);
			
			subCategoryDetails.add(categorySubCategoryInfo);
		}
		if (subCategoryDetails.size() == 1) {
			categorySubCategoryInfo = new CategorySubCategoryDetails();
			categorySubCategoryInfo.setCategoryId(-1);
			categorySubCategoryInfo.setCategoryName("");
			categorySubCategoryInfo.setSubcategoryId(-1);
			categorySubCategoryInfo.setSubCategoryName("");
			categorySubCategoryInfo.setWeightage(-1);
			categorySubCategoryInfo.setActive(false);
			subCategoryDetails.add(categorySubCategoryInfo);
		}
		session.close();
	...
	public int saveSubCategory(SaveSubCategoriesRequest saveSubCategoriesRequest) throws DuplicateAreasException, NoRecordsFoundException {
		...
		if(saveSubCategoriesRequest.getSubCategoryId() != -1){
			...
			subCategoryEntity.setQuestionnaireTypeId(saveSubCategoriesRequest.getQuestionnaireType());
